...
import { CartItem, MutateCartFnInput } from '@/types/cart-types.ts'
import { MinusSignIcon, PlusSignIcon } from '@hugeicons/core-free-icons'

const fetchCartItems = createServerFn({ method: 'GET' }).handler(async () => { // Función para obtener los items del carrito
  const { getCartItems } = await import('@/data/cart.server.ts')
...
function CartPage() {
  const queryClient = useQueryClient();                                 // Cliente de query 
  const router = useRouter()                                            // router para navegar entre las rutas    
  const cart = Route.useLoaderData()                                    // Datos del carrito
  const shipping = cart.items.length > 0 ? 8 : 0                        // Costo de envío
  const subtotal = cart.items.reduce(                                   // Subtotal del carrito (precio * cantidad)
    (acc: number, item) => acc + Number(item.price) * item.quantity,
    0,
  )
  const total = subtotal + shipping                                    // Total del carrito (subtotal + costo de envío)

  if (cart.items.length === 0) {
    return <EmptyCartState />
  }

  const handleClearCart = async () => {                                // Función para limpiar el carrito
    await mutateCartFn({
      data: {
        action: 'clear',
      },
